...
export default function UserItem({ user }) {
	...
	return (
		<List.Item rowKey={user.id} className={usercss["cl"]} style={{ background: user.isActive ? "#18191f" : null }} key={user.id}>
			<List.Item.Meta
				...
				description={
					<Text className={usercss["cl__msg"]} style={user.typing ? { color: "#258C60" } : null}>
						{user.msg_type === "video" ? <VideoIcon style={{ marginRight: "3px" }} /> : null}
						{user.msg_type === "file" ? <FileIcon style={{ marginRight: "3px" }} /> : null}
						{user.msg_type === "audio" ? <AudioIcon style={{ marginRight: "3px" }} /> : null}
